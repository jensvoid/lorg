...
{	
	...
	f!=undefined && f["btn_text"]!=undefined ? btntext=f["btn_text"] : btntext="find";//defines button text
	...
	f!=undefined && f["rows_counter_text"]!=undefined ? totrows_text=f["rows_counter_text"] : totrows_text="Incidents found: ";//defines rows counter text
	...
	if(fltgrid)
	{
		...
		for(var i=0; i<n; i++)// this loop adds filters
		{
			...
			//hide 'useless' / zero-information columns
			for (var col_index = 0; col_index < useless_cols.length; col_index++)
				if (useless_cols[col_index] == i)
				{
					// mark all cell elements of a certain col invisible
					for (var row=0; row<t.rows.length; row++)
						t.rows[row].cells[i].style.display = "none";
				}
